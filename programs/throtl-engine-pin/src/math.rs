//! Fixed-point virtual-position accounting — pure integer math, host-testable, no framework deps.
//!
//! Copied VERBATIM from the Anchor build's `math.rs` (it had no Anchor dependency). Conventions
//! (also implemented identically in `throtl_core` Dart; cross-checked by shared test vectors —
//! ARCHITECTURE §2.2):
...
mod tests {
    ...
    const P1: u64 = 100 * PRICE_SCALE as u64;
    ...
    fn saturation_does_not_panic_on_extremes() {
        let _ = unrealized_pnl_6(i64::MIN, 1, u64::MAX);
        let _ = settle_to_target(i64::MAX, 1, i64::MIN, u64::MAX);
    }
}
